...
"""Read-only MCP stdio server for rtime-hub connector diagnostics."""
...
from .cli import contacts, doctor, panel, resolve_hub_root, scan_hub


PROTOCOL_VERSION = "2025-11-25"
SERVER_NAME = "rtime-hub-connector"
...
def _hub_root_argument(arguments: JsonObject) -> Path:
    root = resolve_hub_root(_path_argument(arguments, "root", required=False))
    if root is None:
        raise ToolError("rtime-hub root not found; pass root or set RTIME_HUB_ROOT")
    if not root.is_dir():
        raise ToolError("rtime-hub root is not a directory", data={"root": str(root)})
    return root


class RtimeHubConnectorMCP:
    def tools(self) -> list[JsonObject]:
        root_arg = {
            "root": {
                "type": "string",
                "description": "rtime-hub root. Defaults to RTIME_HUB_ROOT or known Mac/orangepi paths.",
            }
        }
        scan_args = {
            **root_arg,
            ...
            "sample_limit": {
                "type": "integer",
                "minimum": 0,
                "description": "Maximum sample cards to return per section.",
            },
        }
        return [
            {
                "name": "hub.doctor",
                "title": "Hub Doctor",
                "description": "Read-only check of local rtime-hub and connector surfaces.",
                "inputSchema": _schema(root_arg),
            },
            {
                "name": "hub.scan",
                "title": "Hub Scan",
                "description": "Scan rtime-hub project, device, contact, service, task, and deployment sections.",
                "inputSchema": _schema(scan_args),
            },
            {
                "name": "hub.panel",
                "title": "Hub Panel",
                "description": "Return card-style project/device/contact/service/task panels without file bodies.",
                "inputSchema": _schema(scan_args),
            },
            {
                "name": "hub.contacts",
                "title": "Hub Contacts",
                "description": "Return contact-directory style cards from rtime-hub without file bodies.",
                "inputSchema": _schema(scan_args),
            },
        ]
        ...
        if method == "initialize":
            ...
            return _response(
                request_id,
                {
                    ...
                    "serverInfo": {
                        "name": SERVER_NAME,
                        "title": "Rtime Hub Connector",
                        "version": SERVER_VERSION,
                        "description": "Read-only project, device, and contact panel diagnostics for rtime-hub.",
                    },
                    "instructions": (
                        "Use read-only hub tools for context and review only. This server does not "
                        "edit hub files, commit changes, sync devices, or read secret file bodies."
                    ),
                },
            )
    ...
    def call_tool(self, name: str, arguments: JsonObject) -> JsonObject:
        handlers: dict[str, Callable[[JsonObject], JsonObject]] = {
            "hub.doctor": self._tool_doctor,
            "hub.scan": self._tool_scan,
            "hub.panel": self._tool_panel,
            "hub.contacts": self._tool_contacts,
        }
        ...
        run_id = f"hub-mcp-{uuid.uuid4().hex}"
    ...
    def _tool_scan(self, arguments: JsonObject) -> JsonObject:
        root = _hub_root_argument(arguments)
        max_files = int(arguments.get("max_files", 50000))
        sample_limit = int(arguments.get("sample_limit", 20))
        return scan_hub(root, max_files=max_files, sample_limit=sample_limit)

    def _tool_panel(self, arguments: JsonObject) -> JsonObject:
        root = _hub_root_argument(arguments)
        max_files = int(arguments.get("max_files", 50000))
        sample_limit = int(arguments.get("sample_limit", 20))
        return panel(root, max_files=max_files, sample_limit=sample_limit)

    def _tool_contacts(self, arguments: JsonObject) -> JsonObject:
        root = _hub_root_argument(arguments)
        max_files = int(arguments.get("max_files", 50000))
        sample_limit = int(arguments.get("sample_limit", 20))
        return contacts(root, max_files=max_files, sample_limit=sample_limit)
    ...
    def _record_run(
        ...
        *,
        ...
        log_path_raw = os.environ.get("RTIME_HUB_MCP_RUN_LOG")
        if not log_path_raw:
            return
...
def serve_stdio(server: RtimeHubConnectorMCP | None = None) -> int:
    server = server or RtimeHubConnectorMCP()
