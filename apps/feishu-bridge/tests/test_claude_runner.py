...
def test_run_claude_passes_allowed_tools(monkeypatch):
    proc = FakeProc([
    ...
    text, session_id, _ = asyncio.run(run_claude("hi", allowed_tools=["Read"]))

    assert text == "ok"
    assert session_id == "sid_1"
    assert "--allowedTools" in captured["args"]
    assert captured["args"][captured["args"].index("--allowedTools") + 1] == "Read"


def test_run_claude_passes_disallowed_tools(monkeypatch):
    proc = FakeProc([
        b'{"type":"result","session_id":"sid_1","result":"ok"}\n',
    ])
    captured = {}
    ...
    text, session_id, _ = asyncio.run(
        run_claude("hi", disallowed_tools=["CronCreate", "CronDelete", "CronList"])
    )
    ...
    assert (
        captured["args"][captured["args"].index("--disallowedTools") + 1]
