...
async def test_concurrent_messages_different_groups():
    ...
    event_group_a.event.message.mentions = [Mock(key="@_bot")]
    ...
    event_group_b.event.message.mentions = [Mock(key="@_bot")]
...
async def test_same_group_messages_serialized():
    ...
    event1.event.message.mentions = [Mock(key="@_bot")]
    ...
    event2.event.message.mentions = [Mock(key="@_bot")]
    ...
    with patch('main._process_message', new_callable=AsyncMock) as mock_process:
        ...
        # 验证使用了同一个锁
        assert not _chat_locks["group_a"].locked()  # 锁已释放
