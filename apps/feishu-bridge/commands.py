...
from bot_config import CLAUDE_CLI, DEFAULT_CWD, MODEL_ALIASES
from session_store import SessionStore, scan_cli_sessions
...
`/model [名称]` 或 `/models` — 查看/切换模型（配置别名或完整 ID）
...
BOT_COMMANDS = {
    "help", "h", "new", "clear", "resume", "model", "models", "mode", "status", "cd", "ls",
    "workspace", "ws", "skills", "mcp", "usage", "stop",
}


def _split_csv_env(name: str, defaults: set[str]) -> set[str]:
    raw = os.getenv(name, "")
    if not raw.strip():
        return set(defaults)
...
def _ustc_chat_models() -> set[str]:
    # Defaults come from the model registry (routing set); RTIME_USTC_MODELS overrides.
    return _split_csv_env("RTIME_USTC_MODELS", model_routing.ustc_chat_models())


def _deepseek_code_models() -> set[str]:
    return _split_csv_env("RTIME_DEEPSEEK_CODE_MODELS", model_routing.deepseek_code_models())


def _qwen_code_models() -> set[str]:
    return _split_csv_env("RTIME_QWEN_CODE_MODELS", model_routing.qwen_code_models())


def _ollama_models() -> set[str]:
    return _split_csv_env("RTIME_OLLAMA_MODELS", model_routing.ollama_models())


def _is_ustc_chat_model(model: str) -> bool:
    return model in _ustc_chat_models()
...
def _is_deepseek_code_model(model: str) -> bool:
    return model in _deepseek_code_models()


def _is_qwen_code_model(model: str) -> bool:
    return model in _qwen_code_models()


def _model_capability_note(model: str) -> str:
    if not model:
        return "Kimi Code：走 Claude Code 包装器路径，保留工具调用、MCP、权限模式和审计能力。"
    if _is_deepseek_code_model(model):
        return "DeepSeek Code：走 Claude Code 包装器路径，工具调用、MCP、权限模式与 Kimi 路径一致。"
    if _is_qwen_code_model(model):
        return "Qwen Code：走 Claude Code 包装器路径，工具调用、MCP、权限模式与 Kimi 路径一致。"
    if _is_ustc_chat_model(model):
        return (
            "USTC chat 模型：默认经 LiteLLM 走 Claude Code 工具路径（工具调用、MCP 与 Kimi 路径一致）；"
            "部署侧设 RTIME_USTC_AGENT=0 时退回纯聊天直连（不执行工具）。"
        )
    if _is_ollama_model(model):
        return "本地 Ollama 模型：走 Claude Code 包装器路径（全工具），但 Jetson 逐条预填很慢（分钟级），只适合离线兜底/实验。"
    return "Claude Code/Kimi 路径：适合改代码、读写文件、跑测试、调用工具。"


def _model_buttons(chat_id: str) -> list[dict]:
    labels = {
        "opus": "Opus",
        "sonnet": "Sonnet",
        "haiku": "Haiku",
        ...
        "ds-code": "DeepSeek Code",
        "deepseek-coder": "DeepSeek Code",
        "qwen-code": "Qwen Code",
        "qwen-coder": "Qwen Code",
        "ds": "USTC DeepSeek Chat",
        "deepseek": "USTC DeepSeek Chat",
        "qwen": "USTC Qwen Chat",
        "qianwen": "USTC Qwen Chat",
        "qwen-reasoner": "USTC Qwen 推理",
        "ollama": "本地 Ollama",
        "qwen-local": "本地 Ollama",
    }
    buttons = []
    aliases: list[str] = []
    for alias in PREFERRED_MODEL_ALIASES:
    ...
    for alias in aliases:
        ...
        dedupe_key = target or alias
        if dedupe_key in seen_targets:
            continue
        seen_targets.add(dedupe_key)
        label = labels.get(alias, alias)
        buttons.append({
            "text": label,
            "value": {"action": "run_cmd", "cmd": f"/model {alias}", "cid": chat_id},
        })
    return buttons


def _model_menu_text(current_model: str) -> str:
    current = current_model or "(wrapper default)"
    return (
        f"当前模型：**{current}**\n"
        f"{_model_capability_note(current_model)}\n\n"
        "常用：`/model kimi`、`/model deepseek-code`、`/model qwen-code`、"
        "`/model ds`、`/model qwen`。"
    )
...
async def handle_command(
    ...
    if cmd in ("help", "h"):
        ...
        if not args:
            cur = await store.get_current(user_id, chat_id)
            return {
                "text": _model_menu_text(cur.model),
                "buttons": _model_buttons(chat_id),
            }
        model = MODEL_ALIASES.get(args.lower(), args)
        await store.set_model(user_id, chat_id, model)
        display = model or "(wrapper default)"
        return f"✅ 已切换模型为 `{display}`\n{_model_capability_note(model)}"

    elif cmd == "status":
        cur = await store.get_current_raw(user_id, chat_id)
        sid = cur.get("session_id") or "（新 session）"
        model = cur.get("model")
        model_display = model if model else "(wrapper default)"
        cwd = cur.get("cwd", "~")
